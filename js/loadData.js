...
function prepareData() {
  ...
  theData.theData.forEach(function (d) {
    var jd = JSON.parse(
      LZString.decompressFromEncodedURIComponent(d.jsonstring),
    );

    d.theDateTime = luxon.DateTime.fromFormat(
      d.timestamp,
      "yyyy-MM-dd HH:mm:ss",
    );
  ...
  theData.theData.forEach(function (d) {
    theData.dataKeys.forEach(function (e) {
      if (
        typeof d.jsondata[e] == "undefined" ||
        d.jsondata[e] == null ||
        d.jsondata[e] == "no answer"
      ) {
        d.jsondata[e] = "";
      }
    });
  ...
  $.getJSON(
    "data/ritual.json?v=" + Math.random() * Math.random(),
    function (data) {
...
function prepareLegend() {
  d3.select("#legend").style(
    "width",
    d3.select("#vizholder svg").style("width"),
  );
  ...
  ritualDefinition.datatocollect.forEach(function (d) {
    if (d.type == "switch" || d.type == "select") {
      ...
      var la = legendHolder.append("div").attr("class", "legendarea");

      la.append("div").attr("class", "legenheader").text(fieldlabel);

      d.positions.forEach(function (e) {
        var litemcontainer = la.append("div").attr("class", "litemcontainer");

        litemcontainer
          .append("div")
          .attr("class", "licolor")
          .style("background", colorschemes[fieldid](e[0]));
        litemcontainer.append("div").attr("class", "lilabel").text(e[0]);
      });
    }
  });
...
function drawVViz() {
  svg = d3
    .select("#vizholder")
    .append("svg")
    .attr("width", totalwidth + 2 * margin)
    .attr("height", totalheight + 2 * margin);
  gtot = svg
    .append("g")
  ...
  while (maxdate.diff(previoushour) > 0) {
    ...
    theData.theData.forEach(function (d) {
      if (
        d.theDateTime.diff(previoushour).valueOf() >= 0 &&
        d.theDateTime.diff(thishour).valueOf() <= 0
      ) {
        ...
        ritualDefinition.datatocollect.forEach(function (e) {
          ...
          if (
            typeof d.jsondata[fid] != "undefined" &&
            d.jsondata[fid] != null
          ) {
...
function text_object(text_rep, id) {
  ...
  this.setFontSize = function (fontsize) {
    ...
    this.text_object
      .attr("font-size", this.font_size + "pt")
      .attr("dy", this.dy);
  ...
  this.scale = function (width_, height_) {
    ...
    this.text_object.attr(
      "transform",
      "scale(" + this.scale_.a + "," + this.scale_.b + ")",
    );
  };
  ...
  this.wrapTextObject = function (content) {
    ...
    if (t_wrapped.length > 1) {
      ...
      for (var i = 1; i < t_wrapped.length; i++) {
        this.text_object
          .append("tspan")
          .attr("x", parseFloat(this.text_object.attr("x")))
          .attr("dy", ".85em")
          .attr("y", parseFloat(this.text_object.attr("y")) + text_height * i)
          .text(t_wrapped[i]);
      }
    }
  };
